...
Page({
  ...
  getDatapoints: function () {
    return new Promise((resolve, reject) => {
      wx.request({
        url: `https://api.heclouds.com/devices/${devicesId}/datapoints?datastream_id=Number&limit=20`,
        /**
         * 添加HTTP报文的请求头, 
         * 其中api-key为OneNet的api文档要求我们添加的鉴权秘钥
         * Content-Type的作用是标识请求体的格式, 从api文档中我们读到请求体是json格式的
         * 故content-type属性应设置为application/json
         *Number&limit=20表示从Number数据流中读取20个数据
         */
        header: {
        ...
        success: (res) => {
          ...
          resolve({
            ...
            number: response.data.datastreams[0].datapoints.reverse(),
  ...
  convert: function (datapoints) {
    ...
    var length = datapoints.number.length
    for (var i = 0; i < length; i++) {
      categories.push(datapoints.number[i].at.slice(5, 19));
      number.push(datapoints.number[i].value);
